...
// @formatter:off
/**
 * The staged-key batched FP16 paged attention against the score-reusing kernel it was copied from,
 * on the device, over identical inputs: the stored causal scores and the final outputs, raw-bit
 * equal, over NaN-poisoned scratch and outputs.
 *
 * <p>The candidate changes only how the first pass reads keys — through a transposed shared tile
 * instead of one uncoalesced row per lane — while each lane still accumulates its own position's
 * dot product over the dimensions in the reference order, so the scores it stores and everything
 * downstream must carry the same bits.
 *
 * <p>Cases cover the tile boundaries the staging introduces (positions 127, 128 and 129 within a
 * chunk, and ranges shorter than one tile), page boundaries with shuffled physical pages, a second
 * layer and a non-zero slot, partial chunks, width 64, and ranges through position 2047.
 */
// @formatter:on
public class Qwen35AttentionStagedParityAccelTest {
    ...
    private static void assertBitParity(
        ...
        scheduler.addWorkerGrid("attn.staged", grid(rows));
    ...
    /** Ranges ending at positions 100..131: the 127/128/129 tile boundary inside one chunk. */
    @Test
    public void theTileBoundaryAgreesBitForBit() throws Exception {
    ...
    public void screen() throws Exception {
        ...
        for (int[] c : cases) {
            ...
            try (TornadoExecutionPlan planA = new TornadoExecutionPlan(a.snapshot());
                ...
                report("scored rows=" + rows + " start=" + startPos, tA);
                report("staged rows=" + rows + " start=" + startPos, tB);
            }
        }
    }
