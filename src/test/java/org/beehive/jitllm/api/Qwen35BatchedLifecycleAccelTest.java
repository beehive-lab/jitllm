...
// @formatter:off
/**
 * The same lifecycle with the prompt ingested in chunks: load, session, generate, reset, close.
 *
 ...
 *
 * <p>Batched ingestion makes the reset question sharper, not softer. The chunks advance the same
 * recurrent buffers the decode graphs then read, and a reset that cleared only the host copy — or
 * only what the single-token path touches — would leave the second turn conditioned on the first.
 *
...
public class Qwen35BatchedLifecycleAccelTest {
    ...
    @Test
    public void aBatchedSessionGeneratesResetsAndClosesOnTheDevice() throws Exception {
